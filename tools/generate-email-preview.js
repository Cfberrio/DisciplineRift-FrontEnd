// Script genérico para generar preview de correos de recordatorio de 1 semana
// Uso: node generate-email-preview.js [YYYY-MM-DD]
...
async function generatePreview() {
  try {
    // Obtener fecha del argumento de línea de comandos o usar fecha por defecto
    const targetDate = process.argv[2] || '2025-09-19';
    
    // Validar formato de fecha
    if (!/^\d{4}-\d{2}-\d{2}$/.test(targetDate)) {
      console.error('❌ Formato de fecha inválido. Usar YYYY-MM-DD');
      process.exit(1);
    }
    
    console.log(`🔍 Generando preview del correo de recordatorio de 1 semana para ${targetDate}...`);
    ...
    // Intentar procesar cada sesión hasta encontrar una con inscripciones activas
    for (let i = 0; i < sessions.length; i++) {
      const session = sessions[i];
      console.log(`\n🏐 Procesando sesión ${i + 1}/${sessions.length}: ${session.sessionid} del equipo ${session.teamid}`);
      ...
      if (enrollmentsError || !enrollments || enrollments.length === 0) {
        console.log(`⚠️ No hay inscripciones activas para este equipo, probando siguiente...`);
        continue;
      }
      ...
      if (studentsError || !students) {
        console.log(`⚠️ No se encontró el estudiante, probando siguiente...`);
        continue;
      }
      ...
      if (parentsError || !parent) {
        console.log(`⚠️ No se encontró el padre, probando siguiente...`);
        continue;
      }
      ...
      const fileName = `email-preview-${cleanDate}.html`;
    ...
    console.log('❌ No se pudo generar preview: no hay datos completos para ninguna sesión');
