...
/**
 * Función principal para enviar correos de registro
 */
async function sendRegistrationEmails() {
  ...
  try {
    ...
    // 2. Obtener todos los padres de la tabla Enrollment (isactive true o false)
    ...
    // Resumen final
    console.log('\n' + '='.repeat(60));
    console.log('📊 REGISTRATION EMAIL CAMPAIGN SUMMARY');
    console.log('='.repeat(60));
    console.log(`✅ Total emails sent successfully: ${successCount}`);
    console.log(`❌ Total errors: ${errorCount}`);
    console.log(`📧 Unique recipients: ${emailsSent.size}`);
    console.log('='.repeat(60) + '\n');

  } catch (error) {
    console.error('❌ Fatal error in sendRegistrationEmails:', error);
    throw error;
  }
}
...
if (require.main === module) {
  sendRegistrationEmails()
    .then(() => {
      console.log('✅ Registration email campaign completed successfully');
      process.exit(0);
    })
    .catch((error) => {
      console.error('❌ Registration email campaign failed:', error);
      process.exit(1);
    });
}

export { sendRegistrationEmails };
