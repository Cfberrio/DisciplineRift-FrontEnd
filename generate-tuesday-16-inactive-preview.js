// Script para generar preview del correo de recordatorio del día anterior para Tuesday 16 September 2025
// Con condición adicional: team.isactive = false
...
// Template del email de recordatorio del día anterior
...
async function generatePreview() {
  try {
    console.log('🔍 Generando preview del correo para Tuesday 16 September 2025...');
    console.log('📋 Condiciones: enrollment.isactive = true AND team.isactive = false');
    ...
    // Buscar sesiones que inician en esa fecha con equipos inactivos
    ...
    console.log(`📊 Encontradas ${sessions?.length || 0} sesiones con equipos inactivos`);

    if (!sessions || sessions.length === 0) {
      console.log('❌ No hay sesiones con equipos inactivos para esa fecha');
      return;
    }
    ...
    console.log(`🔸 Estado del equipo: isactive = ${session.team.isactive}`);
    ...
    // Guardar el preview
    const fs = require('fs');
    const fileName = 'real-tuesday-16-inactive-preview.html';
    ...
    console.log(`🔸 Estado del equipo: isactive = false`);
    console.log(`🔸 Estado de inscripción: isactive = true`);
